...
let test_member () =
  ...
  Alcotest.(check (result (option yaml) error)) "success" ok_output (member "author" ok_input);
  Alcotest.(check (result (option yaml) error))  "fail" err_output (member "author" err_input)
...
let test_keys () =
  ...
  let err_input = `String "Test_Error" in
  let err_output = Error (`Msg "Expecting a Yaml.value of `O") in
  Alcotest.(check (result (keys_compare) error)) "success" ok_output (keys ok_input);
  Alcotest.(check (result (keys_compare) error))  "fail" err_output (keys err_input)
...
let test_to_list () =
  ...
  let err_input = `String "Test_Error" in
  let err_output = Error (`Msg "Expecting a Yaml.value of `O") in
  Alcotest.(check (result (values_compare) error)) "success" ok_output (to_list ok_input);
  Alcotest.(check (result (values_compare) error))  "fail" err_output (to_list err_input)
...
let test_to_string () =
  ...
  Alcotest.(check (result (string_compare) error)) "success" ok_output (to_string ok_input);
  Alcotest.(check (result (string_compare) error))  "fail" err_output (to_string err_input)
...
let test_to_bool () =
  ...
  Alcotest.(check (result (bool_compare) error)) "success" ok_output (to_bool ok_input);
  Alcotest.(check (result (bool_compare) error))  "fail" err_output (to_bool err_input)
...
let test_to_float () =
  ...
  Alcotest.(check (result (float_compare) error)) "success" ok_output (to_float ok_input);
  Alcotest.(check (result (float_compare) error))  "fail" err_output (to_float err_input)


(* let pp_print_value_string ppf (`String x) = *)
...
(* let pp_print_map ppf = function *)
(*   | `A lst ->   Format.fprintf ppf "[%a]"  Format.(pp_print_list ~pp_sep:pp_comma pp_print_value_string) lst *)
(*   | _ -> Format.fprintf ppf "%s" "bad" *)

(* let map_equal a b = (a=b) *)
(* let map_compare = Alcotest.testable pp_print_map map_equal *)

(* let test_map () = *)
(*   let upper (`String x) = `String (String.uppercase_ascii x) in *)
...
(*   Alcotest.(check (result (map_compare) error)) "success" ok_output (map upper ok_input); *)
(*   Alcotest.(check (result (map_compare) error))  "fail" err_output (map ~f:upper err_input) *)


(* let test_map () = *)
(*   let upper=function *)
(*     | `String x -> `String (String.uppercase_ascii x) *) 
(*     | _ -> `Null *)
(*     in *)
(*   let ok_input = `A [ (`String "John") ; (`String "Doe") ] in *)
(*   let ok_output = Ok ( `A [ (`String "JOHN") ; (`String "DOE") ] ) in *)
...
(*   let err_output = Error (`Msg "Expecting a Yaml.value of `A") in *)
(*   Alcotest.(check (result (map_compare) error)) "success" ok_output (map upper ok_input); *)
(*   Alcotest.(check (result (map_compare) error))  "fail" err_output (Alpaca.Util.map ~f:upper err_input) *)
